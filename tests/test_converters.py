...
def test_convert_json_by_json_converter(
    adapted_json_data, json_converter, from_json_to_json_file
):
...
def test_convert_rss_by_json_converter(
    adapted_rss_data, json_converter, from_rss_to_json_file
):
...
def test_convert_atom_by_json_converter(
    adapted_atom_data, json_converter, from_atom_to_json_file
):
...
def test_convert_from_json_by_rss_converter(
    adapted_json_data, rss_converter, from_json_to_rss_file
):
    """Test rss converter by json adapted file."""

    converted_data = rss_converter.convert(adapted_json_data)
    formatted_converted_data = " ".join(
        [tag.strip() for tag in str(converted_data).split("\n")]
    ).strip()

    assert formatted_converted_data == from_json_to_rss_file


def test_convert_from_rss_by_rss_converter(
    adapted_rss_data, rss_converter, from_rss_to_rss_file
):
...
def test_convert_from_atom_by_rss_converter(
    adapted_atom_data, rss_converter, from_atom_to_rss_file
):
...
def test_convert_from_json_by_atom_converter(
    adapted_json_data, atom_converter, from_json_to_atom_file
):
...
def test_convert_from_rss_by_atom_converter(
    adapted_rss_data, atom_converter, from_rss_to_atom_file
):
...
def test_convert_from_atom_by_atom_converter(
    adapted_atom_data, atom_converter, from_atom_to_atom_file
):
