...
def test_initialize_args_with_random_order(
    command_line_with_random_order, command_args_parameters
):
    """Test transform options to dict method with random order of options."""

    command_manager = CommandManager(
        command_line_with_random_order[1], command_line_with_random_order[2:]
    )
    command_details = command_manager.initialize_args(
        command_args_parameters[command_manager.command]
    )
...
def test_initialize_args_with_spaces(command_line_with_spaces, command_args_parameters):
    ...
    command_manager = CommandManager(
        command_line_with_spaces[1], command_line_with_spaces[2:]
    )
    command_details = command_manager.initialize_args(
        command_args_parameters[command_manager.command]
    )
