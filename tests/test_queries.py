...
def test_sort_method_in_straight_direction_by_json(parsed_json_data, json_query):
    ...
    sorted_items = [
        item["date_published"]
        for item in json_query.sort("asc", parsed_json_data)["items"]
    ]
...
def test_sort_method_in_reverse_direction_by_json(parsed_json_data, json_query):
    ...
    sorted_items = [
        item["date_published"]
        for item in json_query.sort("desc", parsed_json_data)["items"]
    ]
...
def test_filter_method_by_json(parsed_json_data, json_query):
    ...
    filtered_items = [
        item["author"]["name"]
        for item in json_query.filter("Brent Simmons", parsed_json_data)["items"]
    ]
...
def test_filter_method_with_unexisting_value_by_json(parsed_json_data, json_query):
    ...
    filtered_items = [
        item["author"]["name"]
        for item in json_query.filter("Brent", parsed_json_data)["items"]
    ]
...
def test_limit_method_with_value_greater_than_items_by_json(
    parsed_json_data, json_query
):
    """Test limit method of json query class with parameter greater than items amount."""

    items_before_limit = parsed_json_data["items"]
    items_after_limit = json_query.limit(
        f"{len(items_before_limit)+1}", parsed_json_data
    )["items"]
...
def test_sort_method_in_straight_direction_by_rss(parsed_rss_data, rss_query):
    ...
    sorted_items = [
        item["pubDate"]
        for item in rss_query.sort("asc", parsed_rss_data)["channel"]["item"]
    ]
...
def test_sort_method_in_reverse_direction_by_rss(parsed_rss_data, rss_query):
    ...
    sorted_items = [
        item["pubDate"]
        for item in rss_query.sort("desc", parsed_rss_data)["channel"]["item"]
    ]
...
def test_filter_method_by_rss(parsed_rss_data, rss_query):
    ...
    filtered_items = [
        item["author"]
        for item in rss_query.filter("Brent Simmons", parsed_rss_data)["channel"][
            "item"
        ]
    ]
    unique_filtered_items = set(filtered_items)
...
def test_filter_method_with_unexisting_value(parsed_rss_data, rss_query):
    ...
    filtered_items = [
        item["author"]
        for item in rss_query.filter("Brent", parsed_rss_data)["channel"]["item"]
    ]
...
def test_limit_method_with_value_greater_than_items_by_rss(parsed_rss_data, rss_query):
    ...
    items_before_limit = parsed_rss_data["channel"]["item"]
    items_after_limit = rss_query.limit(
        f"{len(items_before_limit)+1}", parsed_rss_data
    )["channel"]["item"]
...
def test_sort_method_in_straight_direction_by_atom(parsed_atom_data, atom_query):
    ...
    sorted_items = [
        item["published"] for item in atom_query.sort("asc", parsed_atom_data)["entry"]
    ]
...
def test_sort_method_in_reverse_direction_by_atom(parsed_atom_data, atom_query):
    ...
    sorted_items = [
        item["published"] for item in atom_query.sort("desc", parsed_atom_data)["entry"]
    ]
...
def test_filter_method_by_atom(parsed_atom_data, atom_query):
    ...
    filtered_items = [
        item["author"]["name"]
        for item in atom_query.filter("Brent Simmons", parsed_atom_data)["entry"]
    ]
    unique_filtered_items = set(filtered_items)
...
def test_filter_method_with_unexisted_value(parsed_atom_data, atom_query):
    ...
    filtered_items = [
        item["author"]["name"]
        for item in atom_query.filter("Brent", parsed_atom_data)["entry"]
    ]
...
def test_limit_method_with_value_greater_than_items_by_atom(
    parsed_atom_data, atom_query
):
    """Test limit method of atom query class with parameter greater than items amount."""

    items_before_limit = parsed_atom_data["entry"]
    items_after_limit = atom_query.limit(
        f"{len(items_before_limit)+1}", parsed_atom_data
    )["entry"]
